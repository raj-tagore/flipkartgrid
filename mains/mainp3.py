...
class bot:
    ...
    def align(self):
        a1 = self.position
        b1 = all_nodes[self.next_target]
        self.next_target_angle = find_line_angle(b1, a1)
...
all_bots = [bot0, bot1, bot2, bot3, bot4]
...
def run_video():
    ...
    while True:
        ...
        for i in all_bots:
            i.find_bot(corners, ids, corners2, ids2)
            # update position, angle and found = true
            i.draw_path(frame, frame2)
            if i.found:
                if i.available:
                    bots_available.append(i)
    ...
    while True:
        while len(bots_available) > 0:
            for index, bt in enumerate(bots_available):
                ...
                th = Thread(target=bt.run_bot, args=(i, d,))
                th.start()
